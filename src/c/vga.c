...
void set_mode(int idx){
    ...
    for(uint32 y=0;y<VGA_HEIGHT;y++){
        for(uint32 x=0;x<VGA_WIDTH;x++){
            put_pixel_rgb(x,y,0xA8,0x0,0x0);
        }
    }
}

static uint8* get_frame_buffer_segment(){
...
static uint8 get_color_index(uint8 r, uint8 g, uint8 b)
{
...
void put_pixel_rgb(uint32 x,uint32 y,uint8 r, uint8 g, uint8 b){
...
void put_pixel_color(uint32 x,uint32 y,uint8 color_index){
    if((x<0||VGA_WIDTH<=x) || (y<0||VGA_HEIGHT<=y))return;
    uint8* pixelAddr = get_frame_buffer_segment()+VGA_WIDTH*y+x;
